...
# 0. 解析公众号文章（构建前自动执行）
echo "=== 解析公众号文章 ==="
if [ -f "$ROOT/articles.txt" ]; then
  node "$ROOT/scripts/parse-articles.mjs" || echo "⚠️  文章解析失败，使用现有数据继续构建"
else
  echo "⚠️  未找到 articles.txt，跳过文章解析"
fi
node "$ROOT/scripts/generate-sitemap.mjs" || echo "⚠️  sitemap 生成失败，使用现有 sitemap"
echo ""
